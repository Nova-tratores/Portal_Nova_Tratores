...
### 9.10 Modo "contato bloqueado" na ficha e na fila (10/09/2026)

A marcação "Não contatar" **não significa que o cliente pediu**: pode ser decisão da loja (não incomodar certas pessoas, ou não voltar a atender). Por isso a mensagem antiga do painel de ligação ("Este cliente pediu para NÃO ser contatado") foi trocada por um texto neutro, e quem abre a ficha de um cliente marcado vê um **modo bloqueado** (`src/components/feedbacks/atendimento/Bloqueado.tsx`):

- **Ficha do cockpit** (`identidade.nao_contatar`): banner vermelho no topo ("CONTATO BLOQUEADO · NÃO CONTATAR" + explicação de que pode ser pedido do cliente ou decisão da loja, e que só se liga com autorização de quem marcou), faixa vermelha no topo de cada coluna, "CONTATO BLOQUEADO · NÃO CONTATAR" sob o nome, e a coluna da direita vira o `PainelBloqueado` (sem "Iniciar ligação", sem "Registrar sem ligar"; só o botão "✅ Liberar contato"). A ficha **carrega menos**: só Cliente, Por que ligar e Atendimentos anteriores; Roteiro, Máquinas, Serviços, Compras, Funcionários/fazendas e Contatos do WhatsApp ficam escondidos atrás de "Mostrar ficha completa mesmo assim". O que resta é desenhado em **preto e branco** (`filter: grayscale(1)` nos detalhes, nunca nos avisos vermelhos). Deep-links `?oportunidade=`/`?registro=` **não iniciam ligação** para cliente bloqueado.
- **Fila** (cards e lista, ao ligar o chip "🚫 Mostrar bloqueados (não contatar)"): o botão Atender vira o selo vermelho "🚫 BLOQUEADO", o nome ganha "NÃO CONTATAR" em vermelho e os demais campos da linha/card ficam em cinza. O botão "✅ Reativar" da lista continua.
