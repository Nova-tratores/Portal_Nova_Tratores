// Renderização de OS / Pedido de Venda no layout "imitação Omie".
// Extraído de src/app/api/clientes/print/route.ts para poder ser reaproveitado
// fora do runtime do Next (ex.: scripts tsx que baixam a OS direto da Omie).
// NÃO importa next/server — só @supabase/supabase-js e ./contas (imports
// relativos, para o tsx conseguir resolver sem o alias @/).

import { createClient } from "@supabase/supabase-js";
import { contaOmie } from "./contas";
...
export interface OmieAccount { name: string; key: string; secret: string; }
...
export async function omieCall(endpoint: string, call: string, param: Record<string, unknown>, acc: OmieAccount) {
...
export function e(s: string) { return (s || "").replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;").replace(/"/g, "&quot;"); }
...
// ====================== ORDEM DE SERVICO ======================
export async function renderOS(codOS: number, empresa: string, preFetched?: any): Promise<string> {
  const acc = getAccount(empresa);
  const emp = EMP[empresa] || EMP["Nova Tratores"];

  // preFetched: resultado de ConsultarOS já obtido pelo chamador (evita chamar
  // ConsultarOS duas vezes com o mesmo nCodOS — a Omie recusa como "redundante").
  const os: any = preFetched ?? await omieCall("/servicos/os/", "ConsultarOS", { nCodOS: codOS }, acc);
...
// ====================== PEDIDO DE VENDA ======================
export async function renderPV(codPedido: number, empresa: string): Promise<string> {
  const acc = getAccount(empresa);
  const emp = EMP[empresa] || EMP["Nova Tratores"];
