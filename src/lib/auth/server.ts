...
function extrairToken(req: NextRequest): string {
...
export async function autenticar(req: NextRequest): Promise<Autenticado | null> {
  ...
  return { userId, email: data.user.email ?? null, isAdmin, isDev }
}

// Igual a autenticar, mas devolve null se o chamador não for admin.
export async function exigirAdmin(req: NextRequest): Promise<Autenticado | null> {
  const auth = await autenticar(req)
