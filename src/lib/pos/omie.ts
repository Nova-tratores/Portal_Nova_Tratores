...
export async function criarOSNoOmie(idOrdem: string): Promise<{ sucesso: boolean; nCodOS?: number; cNumOS?: string; erro?: string; pedidoVenda?: string; pedidoVendaErro?: string; interna?: boolean }> {
  ...
  try {
    // Ordem INTERNA (decisão 21/08): vai pro Omie NORMAL, igual às outras —
    // a OS é criada lá do mesmo jeito. A única diferença fica mais abaixo:
    // os PPVs vinculados saem como REMESSA em vez de Pedido de Venda
    // (enviarPPVParaOmie com { remessa: isInterno }).
