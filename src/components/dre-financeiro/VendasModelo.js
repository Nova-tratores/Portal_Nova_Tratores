...
//
// COMPARTILHADO por duas rotas (prop `variante`):
//   'dre'       -> /dre-financeiro/vendas-modelo. O layout do modulo
//                  (.../dre-financeiro/layout.js) ja aplica o gate 'financeiro',
//                  a sub-nav e o seletor de conta; o gate aqui e redundante por
//                  consistencia com o padrao do portal.
//   'comercial' -> /vendas-modelo, sistema proprio do grupo Comercial (modulo
//                  'vendas-modelo' no Admin). Fora do layout do DRE nao ha seletor
//                  de conta, entao esta variante desenha a sua propria faixa
//                  (vermelha, cor do grupo) com titulo, "?" de ajuda e o seletor
//                  NOVA/CASTRO/TODAS. O seletor PRECISA viver neste componente:
//                  useDreConta e useState por instancia (so o cookie e comum), e
//                  o fetch depende da `conta` desta instancia.
// Em ambas, useDreConta fornece a conta selecionada para montar as URLs.
//
...
export default function VendasModelo({ variante = 'dre' }) {
  ...
  // DRE: 'financeiro' ou a tela 'dre:vendas-modelo'. Comercial: alem desses, o
  // modulo proprio 'vendas-modelo' (sao os MESMOS dados — quem tem o DRE nao e
  // barrado por ter entrado pela URL do Comercial).
  const podeVer = comercial
    ? (temAcesso('vendas-modelo') || temAcesso('financeiro') || pode('dre', 'vendas-modelo'))
    : (temAcesso('financeiro') || pode('dre', 'vendas-modelo'))
  if (!loading && !loadingPerm && userProfile && !podeVer) return <SemPermissao />
  ...
  if (!comercial) return corpo

  // Variante Comercial: faixa propria (titulo + ajuda + seletor de conta) e o
  // mesmo padding que o layout do DRE da aos filhos (o <main> do portal nao da).
  return (
...
// ===========================================================================
// Faixa da variante 'comercial': porte da "Linha 1" do cabecalho do layout do
// DRE (titulo + "?" + seletor de conta), com a cor do grupo Comercial no lugar
// do verde. Estilos inline de proposito (mesma convencao do layout do DRE, e os
  ...
  return (
    <div style={{
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '8px' }}>
        <span style={{ width: '10px', height: '10px', borderRadius: '50%', background: VERMELHO, display: 'inline-block' }} />
        <span style={{ fontWeight: 700, fontSize: '16px', color: '#1e293b' }}>Vendas por Modelo</span>
        <AjudaTela pathname="/vendas-modelo" cor={VERMELHO} />
      </div>
      <div style={{ display: 'flex', alignItems: 'center', gap: '6px' }}>
        <span style={{ fontSize: '11px', color: '#64748b' }}>Conta</span>
        <div style={{ display: 'inline-flex', borderRadius: '8px', overflow: 'hidden', border: `1px solid ${VERMELHO}` }}>
          {contas.map((c, i) => {
            ...
            return (
              <button
                ...
                style={{
                  ...
                  border: 'none', borderLeft: i === 0 ? 'none' : `1px solid ${VERMELHO}`,
