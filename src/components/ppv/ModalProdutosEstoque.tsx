...
interface ProdutoEstoque { codigo: string; descricao: string; estoque: number; reservados: number; estoqueReal: number; valor: number; cmc: number; conta: string }
        ...
        <div style={{ overflowY: "auto", flex: 1 }}>
          {loading ? (
            ...
            <button key={`${p.conta}-${p.codigo}-${i}`} type="button" onClick={() => onSelect(p.codigo, p.descricao, p.conta, p.valor)}
              ...
              <span style={{ textAlign: "right", fontVariantNumeric: "tabular-nums", color: "#64748b" }}>{p.estoque}</span>
