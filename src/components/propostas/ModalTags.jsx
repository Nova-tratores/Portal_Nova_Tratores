...
const CORES = ['#EA580C', '#dc2626', '#d97706', '#16a34a', '#0891b2', '#2563eb', '#7c3aed', '#be185d', '#64748b', '#1e293b'];

const GRUPOS = [
  { value: 'categoria', label: 'Categoria', color: '#dc2626' },
  { value: 'sistema', label: 'Sistema', color: '#2563eb' },
  { value: 'geral', label: 'Geral', color: '#64748b' },
];

export default function ModalTags({ open, onClose }) {
  ...
  const [gruposAbertos, setGruposAbertos] = useState({ categoria: true, sistema: true, geral: true });

  const carregar = useCallback(async () => {
    setLoading(true);
    const { data } = await supabase.from('proposta_tags').select('*').order('nome');
    setTags(data || []);
    setLoading(false);
  }, []);
  ...
  const toggleGrupo = (g) => setGruposAbertos(prev => ({ ...prev, [g]: !prev[g] }));
      ...
      <div style={{ background: '#fff', borderRadius: 20, width: 600, maxHeight: '85vh', display: 'flex', flexDirection: 'column', overflow: 'hidden', boxShadow: '0 25px 60px rgba(0,0,0,0.2)' }}>
        ...
        <div style={{ padding: '20px 24px', borderBottom: '1px solid #E2E8F0', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
            <div style={{ width: 40, height: 40, borderRadius: 10, background: 'linear-gradient(135deg, #dc2626, #b91c1c)', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#fff' }}>
              <Tag size={20} />
            </div>
            <div>
        ...
        <div style={{ padding: '16px 24px', borderBottom: '1px solid #F1F5F9' }}>
          <div style={{ display: 'flex', gap: 10, alignItems: 'flex-end' }}>
            ...
            <button onClick={criar}
              style={{ padding: '10px 16px', borderRadius: 10, border: 'none', background: '#dc2626', color: '#fff', fontSize: 13, fontWeight: 700, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 5, whiteSpace: 'nowrap' }}>
              <Plus size={14} /> Criar
            </button>
          </div>
