  ...
  try {
    ...
    try {
      ...
      ;(lancCC || []).forEach((r: any) => {
        ...
        // classificaGrupoCP (nao classificaCP): lancamento de CC nao e' titulo APIP;
        // classifica pelo grupo (substitui o antigo objeto grupoCpClassif removido).
        const cls = classificaGrupoCP(r.grupo_categoria); if (!cls) return
        if (!matchesClassif(cls.tipo, cls.grupo, cls.conta)) return
