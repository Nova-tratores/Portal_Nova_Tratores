...
export default function Kanban() {
 ...
 const chamadosFiltrados = chamados.filter(c => !ehFilhoDeGrupo(c)).filter(c => ehDoSetor(c, SETOR_KANBAN)).filter(c => !temNotaServico(c)).filter(c => {
 ...
 return (
  <div style={{ minHeight: 'calc(100vh - 64px)', fontFamily: 'Inter, sans-serif' }}>
   ...
   <AgruparCardsModal
     open={agruparOpen}
     cards={chamados.filter(c => (c.status === 'gerar_boleto' || c.status === 'validar_pix') && !ehFilhoDeGrupo(c) && ehDoSetor(c, SETOR_KANBAN) && !temNotaServico(c))}
