...
export default function MecanicosPage() {
  ...
  if (detalhe) {
    ...
    const ocPendentes = detalhe.ocorrencias.filter(o => o.status === 'pendente').length
...
<div><span style={{ fontSize: 11, fontWeight: 700, color: '#94A3B8', textTransform: 'uppercase' }}>GPS Mes</span><div style={{ fontSize: 13, color: '#1E293B', fontWeight: 600, marginTop: 2 }}>{detalhe.gps.kmMes} km | {detalhe.gps.dias} dias</div></div>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: 28, marginBottom: 28 }}>
          {secoes.map(s => {
            ...
            return (
              <div key={s.id} onClick={() => setSecao(s.id)} style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 8, cursor: 'pointer' }}>
                <div style={{
                  width: 64, height: 64, borderRadius: '50%',
                  background: isActive ? s.gradient : '#F1F5F9',
                  display: 'flex', alignItems: 'center', justifyContent: 'center',
                  color: isActive ? '#fff' : '#94A3B8',
                  transition: 'all .2s',
                  boxShadow: isActive ? `0 4px 16px ${s.color}40` : 'none',
                  border: isActive ? 'none' : '2px solid #E2E8F0',
                  position: 'relative',
                }}>
                  {s.icon}
                  {(s.badge || 0) > 0 && (
                    <span style={{ position: 'absolute', top: -4, right: -4, width: 20, height: 20, borderRadius: '50%', background: '#EF4444', color: '#fff', fontSize: 11, fontWeight: 800, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>{s.badge}</span>
                  )}
                </div>
                <span style={{ fontSize: 12, fontWeight: isActive ? 800 : 600, color: isActive ? s.color : '#64748B' }}>{s.label}</span>
                <span style={{ fontSize: 18, fontWeight: 800, color: isActive ? '#1E293B' : '#94A3B8' }}>{s.count}</span>
              </div>
            )
          })}
        </div>
            ...
            {detalhe.ocorrencias.length === 0 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                {detalhe.ocorrencias.map(oc => {
                  ...
                  const st = STATUS_CORES[oc.status] || STATUS_CORES.pendente
                  return (
                    <div key={oc.id} style={{ background: '#fff', border: '1px solid #E2E8F0', borderRadius: 10, padding: 16 }}>
                      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                        ...
                        <span style={{ padding: '3px 10px', borderRadius: 6, background: st.bg, color: st.text, fontSize: 11, fontWeight: 700 }}>{oc.status}</span>
                        <span style={{ fontSize: 12, color: '#94A3B8', marginLeft: 'auto' }}>{new Date(oc.created_at).toLocaleDateString('pt-BR')} - {oc.criado_por}</span>
                      </div>
                      <div style={{ fontSize: 14, fontWeight: 700, color: '#1E293B', marginBottom: 4 }}>{oc.titulo}</div>
                      {oc.descricao && <div style={{ fontSize: 13, color: '#64748B', marginBottom: 8 }}>{oc.descricao}</div>}
                      {oc.status === 'pendente' && (
                        <div style={{ display: 'flex', gap: 8 }}>
