...
export default function NotasEntradaPage() {
  ...
  return (
    <div style={{ maxWidth: 1200, margin: '0 auto', padding: '20px 24px' }}>
      ...
      {resp && !carregando && (
        <>
          <div style={{ fontSize: '.78rem', color: '#888', marginBottom: 8 }}>{resp.total} notas — página {resp.pagina}/{resp.totalPaginas || 1}</div>
          <div style={{ overflowX: 'auto', background: '#fff', border: '1px solid #eee', borderRadius: 12 }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead><tr>{['NF', 'Série', 'Emissão', 'Emitente', 'Categoria', 'Valor', ''].map((h, i) => <th key={i} style={thStyle}>{h}</th>)}</tr></thead>
              <tbody>
                {resp.notas.map((n) => (
                  <NotaRow key={n.id}
                    n={n}
...
function NotaRow({ n, expandida, onToggle, titulos, onVerContasPagar, onVerDanfe }: {
  n: Nota; expandida: boolean; onToggle: () => void; titulos: Titulo[] | 'loading' | undefined;
  onVerContasPagar: () => void; onVerDanfe: () => void;
}) {
  return (
    <>
