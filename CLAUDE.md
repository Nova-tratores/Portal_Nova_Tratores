...
- **Migration a aplicar no Supabase:** `sql/add-requisicao-origem.sql` — coluna `origem` em `Requisicao`. Sem ela, o registro AUTOMÁTICO de despesa de alimentação ao fechar OS (`src/lib/pos/alimentacao-os.ts`) falha silenciosamente (nunca funcionou até 13/07/2026 — todas as requisições de Alimentação foram manuais).
